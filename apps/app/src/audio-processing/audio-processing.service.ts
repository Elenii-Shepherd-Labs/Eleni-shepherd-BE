...
export class AudioProcessingService {
  ...
  constructor(
    ...
    this.SILENCE_THRESHOLD_MS =
      this.configService.get<number>('AI_SILENCE_THRESHOLD_MS') || 1500;
    this.MIN_AUDIO_LENGTH_BYTES =
      this.configService.get<number>('AI_MIN_AUDIO_LENGTH_BYTES') || 8000;
  }
  ...
  async processAudioChunk(
    ...
    try {
      ...
      if (hasVoice) {
        ...
        if (
          settings.alwaysListen &&
          !settings.awake &&
          buffer.totalBytes >= this.MIN_AUDIO_LENGTH_BYTES / 4
        ) {
          try {
            ...
            const wakeDetected =
              wakeResp && wakeResp.success
                ? (wakeResp.data as { voiceDetected: boolean }).voiceDetected
                : false;
            ...
            if (wakeDetected) {
              ...
              const transResp = await this.sttService.transcribeAudio(
                candidate,
              );
              const transcript =
                transResp && transResp.success
                  ? (transResp.data as { text: string }).text
                  : null;
        ...
        if (tapActive)
          return createAppResponse(true, 'Tap active', { isFinal: false }, 200);
        ...
        if (
          ...
          const completeAudio = Buffer.concat(
            buffer.chunks.map((c) => Buffer.from(c)) as unknown as Uint8Array[],
          );
          this.audioBuffers.delete(sessionId);

          const currentSettings = this.sessionSettings.get(sessionId) || {
            alwaysListen: false,
            awake: false,
          };
          const now = Date.now();
          ...
          if (
            ...
            !currentSettings.awake &&
            !tapActive
          ) {
            this.logger.debug(
              `Ignoring non-wake audio for session ${sessionId} (always-listen waiting)`,
            );
            return createAppResponse(
              true,
              'Ignored non-wake audio',
              { isFinal: false },
              200,
            );
          }
  ...
  async setAlwaysListening(sessionId: string, enabled: boolean) {
    ...
    this.sessionSettings.set(sessionId, settings);
    this.logger.log(
      `Session ${sessionId} alwaysListen=${settings.alwaysListen}`,
    );
  }

  async tapToListen(sessionId: string, durationMs = 10000) {
    const settings = this.sessionSettings.get(sessionId) || {
      alwaysListen: false,
      awake: false,
    };
  ...
  async *textToAudioStream(
    ...
    this.logger.log(
      `Generating audio for session ${sessionId}: ${text.substring(0, 50)}...`,
    );
  ...
  async processCompleteAudio(
    ...
    try {
      ...
      return createAppResponse(
        false,
        'Error processing complete audio',
        null,
        500,
      );
    }
